...
def get_text_data(driver):
    try:
        review_area = find_review_areas(driver)
        if review_area != None: 
            r_text = get_reviews(review_area)
        else: r_text = ''
    except: r_text = ''

    try:
        statement_area = find_statement_areas(driver)
        if statement_area != None: 
            s_text = get_statements(statement_area)
        else: s_text = ''
    except: s_text = ''
...
def info_crawler(url_data,chrome_options, cookies, write_file):
    ...
    #write_file = pd.DataFrame(columns = ['index','text'])
    write_file = pd.read_csv('/home/dhkim/Fragrance/data/text.csv' ,encoding ='utf-8-sig')
    ...
    datas = []
    for i in tqdm(range(2899, len(url_data))):
        data = {}
        ...
        if url not in visited:
            ...
            data['index'] = i
            data['text'] = get_text_data(driver)
            datas.append(data)
                #data['brand'] = url_data.loc[i, 'brand']
                #data[''] = url_data.loc[i, 'name']
...
##                                data[pie_chart+'_count'] = chart_dic['Votes']
 ...
 #                                   else: data[keys] = chart_dic[keys]
            ...
            if len(datas) % 100 == 0:
                ...
                write_file.to_csv('/home/dhkim/Fragrance/data/text.csv' ,encoding ='utf-8-sig',  index=False)
...
if __name__ == '__main__':
    ...
    #url_data = pd.read_csv('/home/dhkim/Fragrance/data/rating_table2.csv', encoding ='utf-8-sig')
